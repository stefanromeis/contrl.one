...
(function(process) {
  ...
  client.use(express.logger({format: 'dev'})).use(subMiddleware).use(express.static(path.join(path.dirname(__dirname)))).listen(8100);
  server.use(express.logger({format: 'dev'})).use(subMiddleware).use(express.static(path.join(path.dirname(__dirname)))).listen(8200);
  module.exports = {
    client: client,
    server: server
  };
